...
function delete_event_rej(str) {
    ...
    //console.log("before delete",registered);
    ...
    popup_window.remove();
    //console.log("after delete",registered);
}
...
/* show the tutorial*/
function show_tutorial(isLogin) {
    window.scrollTo(0, 0);
    $('#tutorial').css('visibility', 'visible');
    $('#mainPart').css('visibility', 'hidden');
    /*from the login to tutorial */
    if (isLogin == true) {
        $('#login').css('visibility', 'hidden');
        current_user = $('#username').val();
    } else {
        /*from signup to tutorial */
        $('#signup').css('visibility', 'hidden');
        $('#login').css('visibility', 'hidden');
        current_user = $('#username_signup').val();
    }
...
function post_event() {
    ...
    new_event['date'] = "" + parseInt(event_date) + "-" + event_month + "-" + 2021;
    ...
    if (pass) {
        ...
        temp = new_event["date"].split("-");
        alert("Your event " + new_event["name"] + " has been posted on " + temp[1] + "/" + temp[0] + "/" + temp[2]);
        document.getElementById("dayev" + temp[0]).click();
    } else {
...
function filter_event() {
    ...
    //$( "#menu1" ).load(self);
    alert("Successfully filtered! Please click the date button to select events.");
    document.getElementById("dayev1").click();
...
function updateRegisterCalendar(date, event_curr, event_index) {
    ...
    var obj_pop = "<span class='popup'> <div class='popuptext' id='myPopup" + temp[1] + temp[0] + event_index + "'>" +
        "<span class='poptitle'> Event Theme: </span> <span class='popbody'>" + event_now.name + "</span><br>" +
        "<span class='poptitle'>Date: </span>" + event_now.name + "<br>" +
        "<span class='poptitle'> Time:  </span>" + event_now.time + "<br>" +
        "<span class='poptitle'> Location:  </span>" + event_now.location + "<br>" +
        "<span class='poptitle'> Type: </span> " + event_now.type + "<br>" +
        "<span class='poptitle'> Number of Participants:  </span>" + event_now.num + "<br>" +
        "<span class='poptitle'> Description:  </span>" + "<br>" + event_now.description + "<br>" +
        "<span class='poptitle'> Contact:  </span>" + event_now.contact +
        "<br><button class='btn btn-warning' onclick=delete_event_rej('reg_button_"+temp[1]+"_"+temp[0]+"_"+event_index+"')>delete</button>"+
        "</div></span>";
...
function resetcurtab(tab) {
    ...
    if (curtab == 'Events') {
        ...
        word_des.html("You can <em style='color:red'>click</em> or use <em style='color:red'>left</em> key to register Events, <em style='color:red'>right</em> key to delete Events from Backpack. ");
        console.log("word des 2");

    } else {
        word_des.html("You can <em style='color:red'>click</em> or use <em style='color:red'>right</em> key to delete Events from Register. ");
